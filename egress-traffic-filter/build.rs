...
/// This crate has a runtime dependency on artifacts produced by the `egress-traffic-filter-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "egress-traffic-filter-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `egress-traffic-filter-ebpf`, so cargo will automatically rebuild us
        // if `egress-traffic-filter-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `egress-traffic-filter-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("egress-traffic-filter-ebpf");
